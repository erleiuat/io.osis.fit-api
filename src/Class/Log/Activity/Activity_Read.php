...
class Activity_Read extends Activity {

    public function readById($id) {

        $stmt = Database::prepare("
            SELECT * FROM " . self::$db_t_log_activity . " WHERE 
            account_id = :account_id AND
            a_log_activity_id = :a_log_activity_id
        ");

        Database::bind($stmt, ['account_id', 'a_log_activity_id'], [$this->account_id, $id]);
    ...
    public function read($from = false, $to = false) {
        ...
        $sql = "SELECT * FROM " . self::$db_t_log_activity . " WHERE account_id = :account_id";
